<div class="min-h-screen bg-base-200">
    ...
    <!-- Hero -->
    <section class="bg-gradient-to-r from-accent/10 to-primary/10 py-12">
        <div class="container mx-auto px-4 text-center">
            <x-icon name="o-tag" class="w-12 h-12 mx-auto text-accent mb-4" />
    ...
    <main class="container mx-auto px-4 py-12">
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($tag->posts()->where('is_published', true)->with(['category', 'tags'])->latest()->paginate(12) as $post)
            ...
            @empty
                <div class="col-span-full text-center py-16">
                    <x-icon name="o-tag" class="w-16 h-16 mx-auto text-base-content/30 mb-4" />
                    <p class="text-base-content/60">هنوز پستی با این برچسب منتشر نشده است.</p>
                </div>
            @endforelse
        </div>

        @if($tag->posts()->where('is_published', true)->count() > 12)
            <div class="mt-8 flex justify-center">
                {{ $tag->posts()->where('is_published', true)->latest()->paginate(12)->links() }}
            </div>
        @endif
    </main>
